...
parser = argparse.ArgumentParser(description='Convert any stream of reads to a pickle file for WISECONDOR, \
defaults are set for the SAM format', formatter_class=argparse.ArgumentDefaultsHelpFormatter)

parser.add_argument('-binsize', type=int, default=1000000, help='binsize used for samples')
parser.add_argument('-retdist', type=int, default=4, help='maximum amount of base pairs difference between sequential \
reads to consider them part of the same tower')
parser.add_argument('-retthres', type=int, default=4, help='threshold for when a group of reads is considered a tower \
and will be removed')
